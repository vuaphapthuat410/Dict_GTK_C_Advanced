...
int commond_char(char * str1, char * str2, int start) // tra lai diem khac nhau dau tien cua 2 chuoi ke tu index start
{
	int i;
...
void jrb_to_list(JRB nextWordArray, int number) // Chua hieu lam
{
	GtkTreeIter Iter;  // Kieu con tro trong GtkTreeModel // kieu cay trong gtk giup luu tru data
  	JRB tmp;
  	int sochia = number / 9;
   	int max = 0;
		...
		if (jrb_empty(nextWordArray)) {   // Ko co tu nao co prefix la word thi se goi y theo kieu soundex
			...
			strcpy(nextword, jval_s(jrb_first(nextWordArray)->key));
		   		jrb_traverse(tmp, nextWordArray) {
					...
					// tinh vi tri diem chung cuoi cua 2 chuoi nextword va tmp->key
   					if (minNumOfCommondChar > NumOfCommondChar)
						minNumOfCommondChar = NumOfCommondChar;   //duyet ca cay de tim minNumOfCommondChar 
				}

				if ((minNumOfCommondChar  != 1000) && (minNumOfCommondChar > wordlen)) {
					nextword[NumOfCommondChar] = '\0';
				   	gtk_entry_set_text(GTK_ENTRY(entry_search), nextword);                        // Kho hieu
			   		gtk_editable_set_position(GTK_EDITABLE(entry_search), NumOfCommondChar);      // Kho hieu vc
			   	}
		   	}
	}
	else
...
static void search(GtkWidget *w, gpointer data) // ham chuc nang search
{
	...
	strcpy(word,a);
	if (word[0] == '\0')		//void Show_message(GtkWidget * parent , GtkMessageType type,  char * mms, char * content)
...
/*gtk_table_attach (GtkTable *table,GtkWidget *child,guint left_attach,guint right_attach,guint top_attach,guint bottom_attach,
...
int main(int argc, char *argv[])
{
    ...
    //anh nen
	image = gtk_image_new_from_file("minion.jpg");
	gtk_container_add(GTK_CONTAINER(fixed), image);

	btn1 = gtk_button_new_with_label("Tra từ");
	gtk_fixed_put(GTK_FIXED(fixed), btn1, 20, 20);   // Dat btn1 vao toa do 20,20
	gtk_widget_set_size_request(btn1, 120, 50);		 // Set kich thuoc btn1  120 width 50 height
...
//		gtk_widget_set_tooltip_text(btn3, "Delete a word");
	...
	btn4 = gtk_button_new_with_label("Thoát");
	gtk_fixed_put(GTK_FIXED(fixed), btn4, 20, 230);
   	gtk_widget_set_size_request(btn4, 120, 50);
	g_signal_connect(G_OBJECT(btn4), "clicked", G_CALLBACK(close_window), window);
